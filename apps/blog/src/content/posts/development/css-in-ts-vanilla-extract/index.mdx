...
**zero runtime**은 vanilla-extract에서 강조하는 주요 특징이다.
...
![code-description-helper](./images/code-description-helper.png)
...
`-webkit-tap-highlight-color`과 같은 벤더 프리픽스(Vender Prefix) 속성은 처음 `-`를 제거하고 `PascalCase`를 사용한다.
...
![header-css-code](/images/posts/css-in-ts-vanilla-extract/header-css-code.gif)
...
![result-code](./images/result-code.png)

만들어진 코드를 구체적으로 살펴보면 Zero runtime과 관련해서 사용된 스타일 코드가 난독화를 거쳐 만들어진다.

**style 태그**

![style-code](./images/style-code.png)

**난독화가 적용된 class 이름**

![classname-code](./images/classname-code.png)
