...
![Supabase와 Next.js 14 관련 트윗](./images/supabase-nextjs14-tweet.png)
...
![Supabase 새 프로젝트](./images/supabase-new-project.png)

앞서 `env.local`에 추가하지 못한 URL과 API Keys 값이 생성된다.
`env.local`에 설정해준다.

![Supabase 프로젝트 URL, API 키](./images/supabase-project-url-api-key.png)

여기까지 잘 진행했으면 프로젝트를 실행했을 때 다음과 같이 `Login` 버튼이 나오게 된다.

![프로젝트 로그인 버튼](./images/project-with-login-button.png)
...
<div style={{ display: 'flex', justifyContent: 'center', gap: '10px' }}>
  <div>
    <span>테이블 생성</span>
    ![notes 테이블 생성](./images/notes-table.png)
  </div>
  <div>
    <span>데이터 추가</span>
    ![notes 데이터 추가](./images/notes-data.png)
  </div>
</div>
...
<video src="/videos/development/quickly-start-a-project-with-supabase/rls-policy.mp4" controls style="max-width:100%;max-height:400px;display:block;margin:1.5rem auto;border-radius:0.5rem;" />
...
![notes 데이터 확인](./images/notes-data-confirm.png)
...
![Supabase 카카오 Providers 설정](./images/supabase-kakao-providers.png)
...
<video src="/videos/development/quickly-start-a-project-with-supabase/project-login-logout.mp4" controls style="max-width:100%;max-height:400px;display:block;margin:1.5rem auto;border-radius:0.5rem;" />
...
![Supabase Database Functions](./images/supabase-database-functions.png)

이렇게 만든 함수를 `auth` 스키마에 따라 트리거시키도록 해야 한다.
하지만 현재 Supabase에서는 `auth` 스키마가 읽기 전용으로 보호되어 있어 다르게 접근해야 한다.

![Supabase Database Triggers Before](./images/supabase-database-triggers-before.png)
...
![Supabase Database Triggers After](./images/supabase-database-triggers-after.png)
...
![Supabase Profiles 테이블 외래키](./images/supabase-profiles-foreign-key.png)

이제 다시 카카오 로그인을 진행해보면 `profiles` 테이블에 유저 데이터가 자동으로 추가된다.

![Supabase Profile 데이터](./images/supabase-profiles-data.png)
...
<video src="/videos/development/quickly-start-a-project-with-supabase/nextjs-with-supabase-playground.mp4" controls style="max-width:100%;max-height:400px;display:block;margin:1.5rem auto;border-radius:0.5rem;" />
