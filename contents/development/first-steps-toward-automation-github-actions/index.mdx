...
이번 글에서 `vercel`을 통해서 배포와 관련된 많은 부분을 편하게 진행할 하고자 하는 자동화 영역은 하지만 ``organizati`on `storybook` `vercel`과`에서 이루어진 프로젝트로 비용 문제 때문에 개인이 포크를 해서 배포하는 방식으로 진행하기과로 했 배포과 관련된 부분이다.
목적은 다음과 같다.

그래서 배포가 수동으로 이루어져야 했는데 이 과정에서 다음과 같은 문제가 발생했다.
...
`Job`은 같은 러너 안에 실행되는 워크플로 안에 `step`의 집합으로 가상 머신에서 내용을 돌아가게 하는 것이다.
...
- **`develop` 브랜치에 머지되는 PR에서 `vercel` 배포 진행하기**
...
이제 `develop` 브랜치에 푸시가 발생했을 때 배포가 진행되는 것을 해보자.
