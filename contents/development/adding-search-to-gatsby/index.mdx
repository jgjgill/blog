...
다행히도 `js-search`보다 더 쉽게 검색 기능을 도와주는 플러그인 존재했습니다.
...
gatsby-plugin-fusejs에서는 **react-use-fusejs**과 함께 사용할 것을 추천합니다.
하지만 저의 경우 react-use-fusejs과 함께 사용할 시 에러가 발생하더라구요.😂
...
해당 과정에서는 플러그인 내부의 코드도 살펴보며 커스터마이징하는 과정을 거쳤습니다. 다행히 성공적으로 구현하여 더 만족스럽네요.
