...
# 혼콕에서는 React Query가 어떻게 쓰이고 있을까?
...
이렇게 하면 배열 데이터에 한해서는 `undefined`를 없어지는 것을 확인할 수 있다.
...
당연히 이러한 방식은 객체와 같은 단일 데이터 형식에서만 적용할 수 있어 문제가 발생했다.
...
`assert` 구문만으로 처리하기에는 아쉬움이 남아서 혼콕 프로젝트 일정이 끝나고 추가적으로 학습을 해보았다.
...
또한, 쿼리 옵션에서 `enabled`가 `false`일 때는 쿼리가 실행되지 않기 때문에 다음과 같은 상황도 고려해주어야 한다.
...
이러한 `useSuspenseQuery`는 `React Query` [버전5](https://tanstack.com/blog/announcing-tanstack-query-v5)에서는 공식적으로 지원하고 있다.
이제 버전5가 주요 버전이 되었으니 향후에는 지원하는 훅을 사용하자.
