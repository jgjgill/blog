...
<Callout>
  💡 Next.js의 getInitialProps에 대해 알아봅니다. 피드백은 언제나 환영입니다:) 11월 12일:
  getInitialProps에서 호출 순서를 잘못 파악한 부분을 파악하여 수정 예정입니다. 죄송합니다.
  😅
</Callout>
...
이때의 동작을 보면 페이지 이동 시 각 페이지의 `getInitialProps` 호출 이후 `_app`의 `getInitialProps`가 호출되는 것을 볼 수 있다.

<br />

그래서 `_app`에서는 각 페이지의 `getInitialProps`에서 주입된 `context`를 받을 수 있다.
...
다국어의 기능이 도입되면서 프로젝트의 서버 동작이 많아지고 있는 상황이다.
