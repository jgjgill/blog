...
번들 형식이 조건부로 로드되는 모듈을 고려할 필요가 없고,

`import`는 `export`를 복사할 필요 없이 직접 참조가 가능하다.
...
여기서 `lodash-es`는 이 최적화 대상 라이브러리 중 하나로,
...
두 라이브러리는 **모듈 포맷**이 다르다는 것이 가장 큰 차이점이다.
