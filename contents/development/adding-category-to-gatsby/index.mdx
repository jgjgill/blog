...
<Callout>
💡 Gatsby에서 카테고리 기능 구현과 리팩토링 과정을 남기고자 합니다.
피드백은 언제나 환영입니다:)
</Callout>
...
<img src="https://raw.githubusercontent.com/jgjgill/blog/main/contents/development/adding-category-to-gatsby/images/graphql-mdx-category.png" alt="graphql-mdx-category" />

특정 mdx 파일뿐만 아니라 전체 mdx 파일에서도 해당하는 카테고리에 대한 데이터를 받아올 수 있습니다.

<img src="https://raw.githubusercontent.com/jgjgill/blog/main/contents/development/adding-category-to-gatsby/images/graphql-allmdx-category.png" alt="graphql-allmdx-category" />
...
`context` 에서 사용되는 값은 `CategoryPage` query에서 지정된 category 게시물 데이터를 가져오는데 사용도비니다.
...
`/category` 경로에 대한 페이지가 필요하면 `/src/pages/category.js`에서 작업하시면 됩니다.
...
단순히 카테고리만 구분하기에는 아쉬움이 많기 때문에 카테고리 옆에 해당하는 게시판 개수를 보여주면 좋을 것 같았습니다.

GraphQL에서 카테고리별로 해당하는 게시물 개수의 데이터를 받아올 수 있습니다.

<img src="https://raw.githubusercontent.com/jgjgill/blog/main/contents/development/adding-category-to-gatsby/images/graphql-category-count.png" alt="graphql-category-count" />
...
그래서 Category 컴포넌트에서 데이터를 불러오는 방식이 더 좋을 것입니다. 관심사 분리에도 용이하구요.

Graphql에서 이와 관련해서 지원해주는 기능이 있었습니다.
`useStaticQuery`로 해당 기능을 사용하면 Category 컴포넌트에서 데이터 처리가 가능합니다.
...
<img src="https://raw.githubusercontent.com/jgjgill/blog/main/contents/development/adding-category-to-gatsby/images/category.gif" alt="category" />
